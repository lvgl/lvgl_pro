...
The LVGL Pro CLI is a command line tool bringing the great power of the Editor to the terminal. It's a Node script, `lvglpro.js`, that generates and compiles C code from your XML, validates projects, runs headless UI tests, and captures screenshots, all without opening the GUI.
...
node lvglpro.js generate path/to/project

# Or pass it explicitly
node lvglpro.js generate path/to/project --token "your-token"
```
...
node lvglpro.js validate path/to/project

# Compile and run all UI tests headlessly
node lvglpro.js run-all-tests path/to/project

# Capture a screen as a PNG
node lvglpro.js screenshot path/to/project screens/home.xml --out home.png
```
...
Run `node lvglpro.js <command> --help` for the full, up-to-date options of any command.
...
```bash
node lvglpro.js generate <project-path> [options]
```
...
```bash
node lvglpro.js compile <project-path> [options]
```
...
```bash
node lvglpro.js validate <project-path> [options]
```
...
```bash
node lvglpro.js run-test <project-path> <testing-file> [options]
node lvglpro.js run-all-tests <project-path> [options]
```
...
```bash
node lvglpro.js screenshot <project-path> <screen> [options]
```
...
```bash
node lvglpro.js compare <generated-path> <reference-path>
```
    ...
    ui-check:
        ...
        steps:
            ...
            - name: Download CLI
              ...
              run: |
                  curl -L https://github.com/lvgl/lvgl_pro/releases/download/nightly/LVGL_Pro_CLI-nightly-linux-mac.zip -o lvglpro.zip
                  unzip lvglpro.zip -d lvglpro

            - name: Generate code
              run: node ./lvglpro/lvglpro.js generate examples -s

            - name: Compile to run tests
              run: node ./lvglpro/lvglpro.js compile examples --target cli

            - name: Run UI tests
              run: node ./lvglpro/lvglpro.js run-all-tests examples
```
...
node lvglpro.js validate path/to/project --errorlimit 25
node lvglpro.js generate path/to/project

# 3: render what the user will actually see
node lvglpro.js screenshot path/to/project screens/home.xml --out home.png --delay 200
```
