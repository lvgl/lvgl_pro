...
The LVGL Pro CLI is a command line tool bringing the great power of the Editor to the terminal. It's an npm package, [`@lvgl/lvglpro`](https://www.npmjs.com/package/@lvgl/lvglpro), that generates and compiles C code from your XML, validates projects, runs headless UI tests, and captures screenshots, all without opening the GUI.
...
That gives you the `lvglpro` command:

```bash
lvglpro --help
```

The package carries its own `lvgl-resources.zip`. On the first run the CLI extracts it into a local cache, so later runs start faster:
...
`generate` and `compile` take `--skip-resources` to skip the hash check and the copy when the cache is already prepared.

### Authentication

The CLI requires a Product or Platform license token, which you can get from your [lvgl.io/pro](https://lvgl.io/pro) dashboard. Provide it in one of two ways:
...
- **Command-line flag:** pass `--token <your-token>` on any command. It takes precedence over the environment variable.
...
<Callout type="warn">
Treat the token like a password. Prefer `LVGLPRO_CLI_TOKEN` over `--token` so it doesn't leak into shell history or CI logs, and never commit it to your repository.
</Callout>
...
The [project's own PR-check workflow](https://github.com/lvgl/lvgl_pro/blob/master/.github/workflows/pr-check.yml) runs the same steps against a real project.
