...
`x1` and `x2` need to be in the `0..1` range, while `y1` and `y2` can be outside it to overshoot. Ready to use curves can be picked from e.g. [easings.net](https://easings.net).
...
/* `NULL` registers it globally. It can be a component's scope as well. */
