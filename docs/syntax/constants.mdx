...
A constant is referenced by its name in `{ }`, where it can also be part of a larger [expression](./expressions). This works in the `view` and in the `<consts>`, `<styles>`, `<subjects>`, `<animations>` and `<gradients>` sections alike:
