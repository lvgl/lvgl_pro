...
- **Subjects** - defined in `globals.xml`. An expression should reference at least one subject (or variant), else it could never update, and it's skipped with a warning.
...
- **`type="subject"` properties** - see below.
...
If the evaluation fails only later — a division by zero after a subject became `0`, for example — a warning is logged and the attribute **keeps its previous value** instead of being set to something meaningless.
