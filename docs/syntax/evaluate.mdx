...
`&&` binds tighter than `||`, and both bind tighter than the ternary, so a range check reads naturally as a condition:
...
<Callout type="warn">
Both sides are **always evaluated** — there is no short-circuiting (same as with the two ternary branches). So `count != 0 and total / count > 5` still fails on a division by zero when `count` is 0.
</Callout>
