...
Any XML attribute value can be an **expression** wrapped in `{ }`. Instead of writing a fixed value, you let LVGL compute it from numbers, colors, strings, constants, and parameters **when the instance is created**.
...
An expression works on a `<style>` element's `enabled` attribute too, so a whole style can be switched on and off. See [Conditional styles](./styles#conditional-styles).
...
Inside the braces you can use literals, operators, constants, [`<prop>`](./api)s, [subjects](./data-binding), and [variants](./variants). In a `{ }` one time evaluation every one of them is simply read once, so their later changes are ignored.

An `@{ }` binding needs to know what to follow, so it reads some of those names differently: a subject or a variant is followed, while a plain `<prop>` is frozen at creation. It also has to reference at least one subject, or it could never update. [Data binding](./data-binding#generic-bindings-with-expressions) covers all of that.

Names are written as **bare identifiers**, without the `$` or `#` prefixes used elsewhere in XML. They are resolved in this order:

1. **Per-instance names**: a [variant](./variants), a `bindable` [`<prop>`](./api), or a local [subject](./data-binding). Each instance has its own, and it shadows a shared name.
...
For the same reason `@{ }` does not work in these sections. They are processed once, so there is nothing to re-evaluate. To make a style react to a subject, put the binding on the `<style>` element in the `<view>` instead, as `enabled="@{...}"` or as a local style property.
