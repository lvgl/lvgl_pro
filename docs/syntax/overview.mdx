...
- **References**: Use `{ }` to reference anything by name: a `<prop>`, a constant, a subject, or a variant (e.g. `width="{card_width}"`). Inside the braces the name is written plain, and it can be part of a larger [expression](./expressions)
- **Bindings**: the same expression, reading a [subject](./expressions) or a [variant](./variants). It is re-evaluated whenever one of them changes; an expression without one is evaluated once, when the element is created
...
- In an [expression](./expressions) a bare name is resolved in a fixed order: per-instance names (a variant, a `bindable` property, a local subject), then property, constant, variant option, subject. The first match wins, so a constant named `temperature` hides a subject of the same name.
...
So this is invalid: `value="I'm here"`. The `'` needs to be escaped as `"I&apos;m here"`.
...
<Callout type="warn">
A `name` is a literal or a `{ }` expression of properties and constants. The Editor has to resolve the name to check that the style, font, image, and so on exists, so it can't wait for a subject.
</Callout>
