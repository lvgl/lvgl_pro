...
- **Bindings**: `@{ }` is the same expression kept alive. It is re-evaluated whenever a [subject](./data-binding) or [variant](./variants) it reads changes, while `{ }` is evaluated once, when the element is created
...
- In an [expression](./evaluate) a bare name is resolved in a fixed order: per-instance names (a variant, a `bindable` property, a local subject), then property, constant, variant option, subject. The first match wins, so a constant named `temperature` hides a subject of the same name.
...
<Callout type="warn">
A `name` is a literal or a `{ }` expression, never a `@{ }` binding. The Editor has to resolve the name to check that the style, font, image, and so on exists, and a binding has no value until it runs.
</Callout>
