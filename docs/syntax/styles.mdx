...
`enabled` takes an [expression](./expressions) like any other attribute value. Reading a [subject](./expressions) or a [variant](./variants) in it makes the style turn itself on and off as that value moves, as above; an expression built from constants and properties, like `enabled="{is_compact}"`, decides once at creation and stays that way.
...
- An expression reading no subject is decided at creation, so the style stays that way for the life of the widget.
...
A local style property takes an expression like any other attribute, selectors included, and reading a subject in it keeps the property up to date. See [Data binding](./expressions#where-the-values-come-from).
...
[Constants](./constants) do have a value there, so a style property can be computed from them with an [expression](./expressions#outside-the-view):
