...
`<bind_style_prop>` copies a subject's value into the property as-is. To compute the value, write an [expression binding](./data-binding#generic-bindings-with-expressions) on the local style property itself, selectors included:
