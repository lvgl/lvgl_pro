...
lv_obj_t * lv_example_chart_ticks_labels_create(void)
{
    ...
    if (lvgl_open_examples_check_target(LVGL_OPEN_EXAMPLES_TARGET_ALL)) {
        ...
        static const int32_t chart_values_0[] = {20, 45, 30, 70, 55, 80};
        lv_chart_set_series_values(chart, lv_chart_series_0, chart_values_0, 6);
        ...
        lv_obj_set_width(scale, lv_pct(90));
        lv_obj_set_height(scale, 28);
        lv_obj_set_style_pad_hor(scale, 48, 0);
        lv_scale_set_mode(scale, LV_SCALE_MODE_HORIZONTAL_BOTTOM);
        lv_scale_set_total_tick_count(scale, 6);
        lv_scale_set_major_tick_every(scale, 1);
        static const char *scale_text_src_0[] = {"Mon", "Tue", "Wed", "Thu", "Fri", "Sat", NULL};
